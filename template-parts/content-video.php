 ...
 */
 ...
 $routex_video_url = function_exists('tpmeta_field')? tpmeta_field('routex_post_video') : '';
...
?>
 ...
 <?php if ( has_post_thumbnail() ): ?>
    <div class="blog__details-thumb mb-20">
        ...
        <?php if ( !empty($routex_video_url) ): ?>
        <div class="postbox__play-btn">
            <a class="popup-video pulse-btn" href="<?php echo esc_url($routex_video_url); ?>"><i
...
<?php else: ?>
<article id="post-<?php the_ID();?>" <?php post_class( 'blog__content mb-80 format-video mb-60' );?>>
    <?php if ( has_post_thumbnail() ): ?>
    <div class="blog__content-thumb position-relative">
        <?php the_post_thumbnail( 'full', ['class' => 'img-responsive'] );?>
        <?php if ( !empty($routex_video_url) ): ?>
        <div class="postbox__play-btn">
            <a class="popup-video zooming" href="<?php echo esc_url($routex_video_url); ?>"> <i
                    class="fa-thin fa-play"></i></a>
        </div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
